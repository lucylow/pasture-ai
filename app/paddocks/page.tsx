...
import { mockPaddocks } from '@/lib/mockData';

// Mock spatial grid points for biomass visualization
const spatialGrid = [
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-xl p-5 shadow-sm border border-slate-100">
            <h3 className="font-semibold text-slate-900 mb-4">My Paddocks</h3>
              ...
              {mockPaddocks.map((p) => (
                <div key={p.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg hover:bg-slate-100 transition-colors">
                  <div>
                    ...
                    <p className="text-[10px] text-slate-500 mt-0.5">{p.areaHa} ha</p>
                  </div>
                  <div className="text-right">
                    <span className="text-pasture-moss font-bold">{p.lastBiomass_gm2} g/m²</span>
                    <p className="text-[10px] text-slate-500">Rest {p.restDays} days</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-white rounded-xl p-5 shadow-sm border border-slate-100">
            <h3 className="font-semibold text-slate-900 mb-4">Top Recommendation</h3>
            {mockPaddocks.length > 0 && (
              <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                <p className="text-sm font-medium text-green-800">{mockPaddocks[0].name}</p>
                <p className="text-sm font-medium text-green-800 mt-1">{mockPaddocks[0].recommendation}</p>
                <p className="text-xs text-green-700 mt-2">Biomass {mockPaddocks[0].lastBiomass_gm2} g/m²</p>
              </div>
            )}
          </div>
